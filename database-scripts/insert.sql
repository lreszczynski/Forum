...
INSERT INTO App_Thread(title, category_id, user_id, active, pinned, create_date)
VALUES ('Admin thread', 1, 5, TRUE, default, now() - INTERVAL '10 DAY'),
...
INSERT INTO App_Post(content, thread_id, user_id, create_date)
VALUES ('Admin post', 1, 5, now() - INTERVAL '10 DAY'),
...
'
DECLARE
    generatedId App_Thread.id%TYPE;
    DECLARE randomTimestamp TIMESTAMP;
BEGIN
for r in 1..100
    loop
    randomTimestamp = NOW() - (random() * (interval ''90 days''));
    INSERT INTO App_Thread(title, category_id, user_id, active, pinned, create_date)
    VALUES (''Random Thread'', 3, 1, TRUE, default, randomTimestamp)
    RETURNING id INTO generatedId;
    UPDATE App_Thread SET title=concat(''Random Thread id='', generatedId) WHERE id = generatedId;
    INSERT INTO App_Post(content, thread_id, user_id, create_date)
    VALUES (''First post'', generatedId, 1, randomTimestamp);
    INSERT INTO App_Post(content, thread_id, user_id, create_date)
    VALUES (''Post Post Post'', 6, 1, NOW() - interval ''1 YEAR'' + (r * (interval ''1 DAY'')))
    RETURNING id INTO generatedId;
    UPDATE App_Post SET content=concat(''Post content id='', generatedId) WHERE id = generatedId;
    END loop;
END;
' LANGUAGE plpgsql;

INSERT INTO App_Post(content, thread_id, user_id, create_date)
VALUES ('Reply by banned user - freshest post', 6, 3, now() - INTERVAL '30 MINUTE');
